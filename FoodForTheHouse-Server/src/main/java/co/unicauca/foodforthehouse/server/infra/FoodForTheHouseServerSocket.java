...
package co.unicauca.foodforthehouse.server.infra;
...
import co.unicauca.foodforthehouse.server.access.Factory;
import co.unicauca.foodforthehouse.server.access.IClienteRepository;
import co.unicauca.foodforthehouse.server.domain.ServiciosCliente;
 ...
 */
public class FoodForTheHouseServerSocket {
    /**
     * Servicio de clientes
     */
    private final ServiciosCliente service;
    /**
    ...
    private static void throwThread() {
        new Thread(new FoodForTheHouseServerSocket()).start();
    }
    ...
    private static void openPort() {
        try {
            ...
            Logger.getLogger(FoodForTheHouseServerSocket.class.getName()).log(Level.SEVERE, "Error del server socket al abrir el puerto", ex);
        }
    }
    ...
    private static void waitToClient() {
        try {
            ...
            Logger.getLogger(FoodForTheHouseServerSocket.class.getName()).log(Level.SEVERE, "Eror al abrir un socket", ex);
        }
    }
    ...
    public void run() {
        try {
            ...
            Logger.getLogger(FoodForTheHouseServerSocket.class.getName()).log(Level.SEVERE, "Eror al leer el flujo", ex);
        }
    }
    ...
    private void processGetCliente(Protocol protocolRequest) {
        ...
        if (cliente == null) {
            ...
            output.println(objectToJSON(cliente));
        }
    }
private void processPostCliente(Protocol protocolRequest) {
     ...
     */
        ...
        cliente.setIdCliente(protocolRequest.getParameters().get(0).getValue());
        cliente.setNombre(protocolRequest.getParameters().get(1).getValue());
        cliente.setCarrera(protocolRequest.getParameters().get(2).getValue());
        cliente.setCalle(protocolRequest.getParameters().get(3).getValue());
        String response = service.createCliente(cliente);
     ...
     */
    private String objectToJSON(Cliente cliente) {
        Gson gson = new Gson();
        String strObject = gson.toJson(cliente);
        return strObject;
    }
}
